...
function readTierConfig(prefix) {
  return {
    ...
    // Postmark message stream (e.g. 'broadcast' for the outbound tier). When
    // set, sent as the X-PM-Message-Stream header. Blank = the server's
    // default transactional stream. Ignored by non-Postmark relays.
    stream: getConfig(`${prefix}_SMTP_STREAM`),
  };
}
...
export async function sendSystemEmail({ to, subject, text, html }) {
  ...
  if (cfg.stream) msg.headers = { 'X-PM-Message-Stream': cfg.stream };
...
export async function sendOutboundEmail({ to, subject, text, html, from }) {
  ...
  const transport = getTransport(cfg);
  const msg = { from: from || cfg.from, to, subject, text, html };
  if (cfg.stream) msg.headers = { 'X-PM-Message-Stream': cfg.stream };
  await transport.sendMail(msg);
}
