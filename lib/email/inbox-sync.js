...
const DEFAULT_LOOKBACK_MS = 6 * 60 * 60 * 1000; // 6 hours on first sync; subsequent syncs pick up from last received
...
export async function syncAccount(account) {
  ...
  let messages;
  try {
    messages = await mail.listInbox({ since, maxResults: 50 });
  } catch (err) {
