 ...
 */
export function createOAuthState({ secretName, clientId, clientSecret, tokenUrl, secretType, returnPath, userId, provider }) {
  // userId + provider are only set for the per-user email-account link flow
  // (secretType === 'email_account'); omitted for the admin agent-secret flow.
  const payload = JSON.stringify({ secretName, clientId, clientSecret, tokenUrl, secretType, returnPath, userId, provider });
