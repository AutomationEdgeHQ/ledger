...
import { auth, requireAuth, requireAdmin } from '../auth/index.js';
...
import {
  ...
  markMessageRead as dbMarkMessageRead,
  markAllReadForUser,
} from '../db/messages.js';
...
/**
 * Get the current user's inbox messages (system DMs only — chat history excluded).
 * @returns {Promise<{messages: object[], hasMore: boolean}>}
 */
export async function getInboxMessages(limit = 25, offset = 0, { unreadOnly = false } = {}) {
  const user = await requireAuth();
  const rows = getMessagesForUser(user.id, { unreadOnly, limit, offset });
  const hasMore = rows.length > limit;
  return { messages: hasMore ? rows.slice(0, limit) : rows, hasMore };
}

/**
 * Get count of unread system DMs for the current user (drives the sidebar badge).
...
/**
 * Mark a single inbox message as read. Ownership verified in the WHERE clause.
 * @param {string} id
 * @returns {Promise<{success: boolean}>}
 */
export async function markMessageReadAction(id) {
  const user = await requireAuth();
  dbMarkMessageRead(id, user.id);
  return { success: true };
}

/**
 * Mark every system DM for the current user as read.
 * @returns {Promise<{success: boolean}>}
 */
...
export async function getChatData(chatId) {
  ...
  if (chat.userId !== user.id) return null;
...
export async function getCodingAgentSettings() {
  ...
  try {
    ...
    const agentModeAutoRun = getConfig('AGENT_MODE_AUTO_RUN') === 'true';
    const codeModeAutoRun = getConfig('CODE_MODE_AUTO_RUN') === 'true';
    ...
    return {
      ...
      modeDefaults: {
        agent: { branch: agentModeBranch, gitAction: agentModeGitAction, autoRun: agentModeAutoRun },
        code: { branch: codeModeBranch, gitAction: codeModeGitAction, autoRun: codeModeAutoRun },
      },
...
/**
 * Set a per-mode default (branch behavior, git action, or auto-run).
 * @param {'agent'|'code'} mode
 * @param {'branch'|'gitAction'|'autoRun'} field
...
export async function setModeDefault(mode, field, value) {
  ...
  try {
    ...
    const keyMap = {
      ...
      'agent.autoRun': 'AGENT_MODE_AUTO_RUN',
      'code.autoRun': 'CODE_MODE_AUTO_RUN',
    };
...
const API_KEY_SECRETS = new Set([
  ...
  'TEAMS_TENANT_ID',
  'ASSEMBLYAI_API_KEY',
  // Email agent — per-firm OAuth app credentials (saved via the Google/
  // Microsoft admin tabs). Stored encrypted; non-secret values are read back
  // for the form via getEmailAppConfig().
...
// ─────────────────────────────────────────────────────────────────────────────
// Email agent — per-user mailbox linking (BYO per-firm OAuth app)
//
// The firm registers its own Google/Azure app and pastes the client creds into
// the admin panel (config keys GOOGLE_OAUTH_* / MS_OAUTH_*). Individual users
// then link their own mailboxes from /profile/email using those firm creds.
// Multiple accounts per user are supported. Internal vs External app type is
// the firm's GCP config and is invisible here — the bot just runs OAuth.
// ─────────────────────────────────────────────────────────────────────────────

// Identity scopes we always append so the callback can resolve which mailbox
// was authorized (see lib/email/account-identity.js).
const EMAIL_IDENTITY_SCOPES = {
  google: ['openid', 'email'],
  microsoft: ['offline_access', 'openid', 'email', 'User.Read'],
};

// Sane defaults if the firm hasn't customized scopes in the admin panel.
const EMAIL_DEFAULT_SCOPES = {
  google:
    'https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/calendar.readonly',
  microsoft: 'Mail.ReadWrite Mail.Send Calendars.Read',
};

function mergeScopes(base, required) {
  const set = new Set((base || '').split(/\s+/).filter(Boolean));
  for (const s of required) set.add(s);
  return Array.from(set).join(' ');
}

/**
 * Read the firm's configured OAuth app credentials for a provider.
 * @returns {{ clientId, clientSecret, scopes, tenantId? }|null} null if unconfigured
 */
async function getFirmEmailAppConfig(provider) {
  const { getConfig } = await import('../config.js');
  if (provider === 'google') {
    ...
    return {
      clientId,
      clientSecret,
      scopes: mergeScopes(getConfig('GOOGLE_OAUTH_SCOPES') || EMAIL_DEFAULT_SCOPES.google, EMAIL_IDENTITY_SCOPES.google),
    };
  }
  if (provider === 'microsoft') {
    const clientId = getConfig('MS_OAUTH_CLIENT_ID');
    const clientSecret = getConfig('MS_OAUTH_CLIENT_SECRET');
    if (!clientId || !clientSecret) return null;
    return {
      clientId,
      clientSecret,
      tenantId: getConfig('MS_OAUTH_TENANT_ID') || '',
      scopes: mergeScopes(getConfig('MS_OAUTH_SCOPES') || EMAIL_DEFAULT_SCOPES.microsoft, EMAIL_IDENTITY_SCOPES.microsoft),
    };
  }
  return null;
}
...
/**
 * Which email providers the admin has configured — drives the connect buttons.
 * @returns {Promise<{ google: boolean, microsoft: boolean }>}
 */
export async function getEmailProviderStatus() {
  await requireAuth();
  return {
    google: !!(await getFirmEmailAppConfig('google')),
    microsoft: !!(await getFirmEmailAppConfig('microsoft')),
  };
}

/**
 * Admin: read the firm's email OAuth app config for the Google/Microsoft admin
 * tabs. Returns non-secret values for prefill + a boolean for whether each
 * client secret is set (never the secret itself), plus the redirect URI the
 * firm must register in their OAuth app.
 */
export async function getEmailAppConfig() {
  await requireAdmin();
  const { getConfig } = await import('../config.js');
  return {
    redirectUri: `${process.env.AUTH_URL}/api/oauth/callback`,
    google: {
      clientId: getConfig('GOOGLE_OAUTH_CLIENT_ID') || '',
      clientSecretSet: !!getConfig('GOOGLE_OAUTH_CLIENT_SECRET'),
      scopes: getConfig('GOOGLE_OAUTH_SCOPES') || '',
      scopesDefault: EMAIL_DEFAULT_SCOPES.google,
    },
    microsoft: {
      clientId: getConfig('MS_OAUTH_CLIENT_ID') || '',
      clientSecretSet: !!getConfig('MS_OAUTH_CLIENT_SECRET'),
      tenantId: getConfig('MS_OAUTH_TENANT_ID') || '',
      scopes: getConfig('MS_OAUTH_SCOPES') || '',
      scopesDefault: EMAIL_DEFAULT_SCOPES.microsoft,
    },
  };
}

/** List the authenticated user's linked mailboxes (no tokens). */
export async function getMyEmailAccounts() {
  const user = await requireAuth();
  try {
    const { listAccounts } = await import('../db/user-email-accounts.js');
...
/**
 * Begin linking a mailbox: build the provider authorize URL using the firm's
 * configured app creds, packing the user id into the encrypted state so the
 * callback can attribute the account without a session in the popup.
 * @returns {Promise<{ url: string }|{ error: string }>}
 */
export async function initiateEmailAccountLink({ provider }) {
  const user = await requireAuth();
  if (provider !== 'google' && provider !== 'microsoft') {
    return { error: 'Unsupported provider' };
  }
  try {
    const cfg = await getFirmEmailAppConfig(provider);
    if (!cfg) {
      return { error: `Your administrator hasn't configured ${provider === 'google' ? 'Google' : 'Microsoft'} email yet.` };
    }
    const { OAUTH_PROVIDERS, msEndpoints } = await import('../oauth/providers.js');
    const { createOAuthState } = await import('../oauth/helper.js');

    const endpoints =
      provider === 'microsoft'
        ? msEndpoints(cfg.tenantId)
        : { authorizeUrl: OAUTH_PROVIDERS.google.authorizeUrl, tokenUrl: OAUTH_PROVIDERS.google.tokenUrl };

    const redirectUri = `${process.env.AUTH_URL}/api/oauth/callback`;
    const state = createOAuthState({
      secretName: `EMAIL_${provider.toUpperCase()}_${user.id}`,
      clientId: cfg.clientId,
      clientSecret: cfg.clientSecret,
      tokenUrl: endpoints.tokenUrl,
      secretType: 'email_account',
      returnPath: '/profile/email',
      userId: user.id,
      provider,
    });
    ...
    return { url: `${endpoints.authorizeUrl}?${params.toString()}` };
  } catch (err) {
    console.error('Failed to initiate email account link:', err);
    return { error: 'Failed to start authorization' };
  }
}

/** Unlink one of the authenticated user's mailboxes. */
export async function unlinkEmailAccount(id) {
  const user = await requireAuth();
  try {
    const { unlink } = await import('../db/user-email-accounts.js');
    unlink(user.id, id);
    return { success: true };
  } catch (err) {
    console.error('Failed to unlink email account:', err);
    return { error: 'Failed to unlink account' };
  }
}

/** Set one of the authenticated user's mailboxes as their default. */
export async function setDefaultEmailAccount(id) {
  const user = await requireAuth();
  try {
    const { listAccounts, setDefault } = await import('../db/user-email-accounts.js');
    if (!listAccounts(user.id).some((a) => a.id === id)) return { error: 'Account not found' };
    setDefault(user.id, id);
    return { success: true };
  } catch (err) {
    console.error('Failed to set default email account:', err);
    return { error: 'Failed to set default' };
  }
}

/** Pause/resume one of the authenticated user's mailboxes. */
export async function setEmailAccountStatus(id, status) {
  const user = await requireAuth();
  if (!['active', 'paused'].includes(status)) return { error: 'Invalid status' };
  try {
    const { listAccounts, setStatus } = await import('../db/user-email-accounts.js');
    if (!listAccounts(user.id).some((a) => a.id === id)) return { error: 'Account not found' };
    setStatus(id, status);
    return { success: true };
  } catch (err) {
    console.error('Failed to update email account status:', err);
...
/**
 * Toggle whether the user's Telegram channel receives system message pushes
 * (e.g. github webhook notifications). The inbox row is always written — only
 * the push to Telegram is gated.
 */
export async function setTelegramSystemMessages(enabled) {
  const user = await requireAuth();
  const { setSystemMessagesEnabled } = await import('../db/user-channels.js');
  setSystemMessagesEnabled(user.id, 'telegram', enabled);
  return { ok: true };
}

// ─────────────────────────────────────────────────────────────────────────────
// Settings — Slack setup flow
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Get Slack status: bot info (if token valid) and configured signing secret.
 * Used by the admin UI to render the current state of each setup step.
 */
export async function getSlackStatus() {
  ...
  try {
    ...
    const { getConfig } = await import('../config.js');
    const { validateBotToken } = await import('../tools/slack.js');

    const botToken = getConfigSecret('SLACK_BOT_TOKEN');
    const signingSecret = getConfigSecret('SLACK_SIGNING_SECRET');

    const appUrl = getConfig('APP_URL');
    const eventsUrl = appUrl
      ? `${appUrl.replace(/\/$/, '')}/api/slack/events`
      : '';
    ...
    if (botToken) {
      ...
      if (v.valid) {
        botInfo = {
          username: v.botInfo.username,
          team: v.botInfo.team,
          teamId: v.botInfo.teamId,
        };
      }
    }
    ...
    return {
      botInfo,
      botTokenSet: !!botToken,
      signingSecretSet: !!signingSecret,
      eventsUrl,
    };
  } catch (err) {
    console.error('Failed to get Slack status:', err);
    return { error: 'Failed to load Slack status' };
  }
}

/**
 * Validate a Slack bot token without saving it.
 */
    ...
    if (result.valid) {
      return {
        ...
        botInfo: {
          ...
          team: result.botInfo.team,
          teamId: result.botInfo.teamId,
        },
      };
    }
    return { valid: false, error: result.error };
...
/**
 * Issue (or re-issue) a Slack verification code for the current user.
 */
export async function issueSlackCode() {
  const user = await requireAuth();
  const { issueCode } = await import('../db/user-channels.js');
  const { getConfig } = await import('../config.js');
  const { validateBotToken } = await import('../tools/slack.js');
  ...
  try {
    const row = issueCode(user.id, 'slack');
...
/**
 * Unlink the current user's Slack channel binding.
 */
export async function unlinkSlackChannel() {
  const user = await requireAuth();
  const { unlink } = await import('../db/user-channels.js');
  unlink(user.id, 'slack');
  return { ok: true };
}

/**
 * Toggle whether the user's Slack channel receives system message pushes.
 */
export async function setSlackSystemMessages(enabled) {
  const user = await requireAuth();
  const { setSystemMessagesEnabled } = await import('../db/user-channels.js');
  setSystemMessagesEnabled(user.id, 'slack', enabled);
  return { ok: true };
}

// ─────────────────────────────────────────────────────────────────────────────
// Settings — Microsoft Teams setup flow
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Get Teams status: configured credentials + messaging endpoint URL.
 */
export async function getTeamsStatus() {
  ...
  try {
    ...
    const appId = getConfigSecret('TEAMS_APP_ID');
    const appPassword = getConfigSecret('TEAMS_APP_PASSWORD');
    const tenantId = getConfigSecret('TEAMS_TENANT_ID');

    const appUrl = getConfig('APP_URL');
    const messagingEndpoint = appUrl
      ? `${appUrl.replace(/\/$/, '')}/api/teams/events`
      : '';
    ...
    if (appId && appPassword) {
      try {
        const { validateCredentials } = await import('../tools/teams.js');
        const result = await validateCredentials(appId, appPassword);
        credentialsValid = result.valid;
        if (!result.valid) validationError = result.error;
      } catch (err) {
    ...
    console.error('Failed to get Teams status:', err);
    return { error: 'Failed to load Teams status' };
  }
}

/**
 * Validate Teams App ID + Password without saving.
 */
export async function validateTeamsCredentials(appId, appPassword) {
  await requireAdmin();
  if (!appId || !appPassword) return { valid: false, error: 'App ID and Password required' };
  try {
    const { validateCredentials } = await import('../tools/teams.js');
    return await validateCredentials(appId, appPassword);
  } catch (err) {
    return { valid: false, error: err.message };
  }
}
...
/**
 * Issue (or re-issue) a Teams verification code for the current user.
 */
export async function issueTeamsCode() {
  const user = await requireAuth();
  const { issueCode } = await import('../db/user-channels.js');
  try {
    const row = issueCode(user.id, 'teams');
    return { code: row.code, expiresAt: row.codeExpiresAt };
  } catch (err) {
    return { error: err.message };
  }
}
...
/**
 * Unlink the current user's Teams channel binding.
 */
export async function unlinkTeamsChannel() {
  const user = await requireAuth();
  const { unlink } = await import('../db/user-channels.js');
  unlink(user.id, 'teams');
  return { ok: true };
}

/**
 * Toggle whether the user's Teams channel receives system message pushes.
 */
export async function setTeamsSystemMessages(enabled) {
  const user = await requireAuth();
  const { setSystemMessagesEnabled } = await import('../db/user-channels.js');
  setSystemMessagesEnabled(user.id, 'teams', enabled);
  return { ok: true };
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Email agent — inbox feed + pipeline control (M4 / M5)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Trigger a full inbox sync + classify + decide cycle for the current user.
 * Returns per-step stats; safe to call frequently (idempotent sync).
 */
export async function runEmailPipeline() {
  const user = await requireAuth();
  try {
    const { runPipeline } = await import('../email/pipeline.js');
    return runPipeline(user.id);
  } catch (err) {
    console.error('runEmailPipeline failed:', err);
    return { error: err.message };
  }
}

/** Send the daily briefing email to the current user on demand. */
export async function sendEmailBriefing() {
  const user = await requireAuth();
  try {
    const { sendBriefingEmail } = await import('../email/briefing.js');
    return sendBriefingEmail(user.id);
  } catch (err) {
    return { error: err.message };
  }
}
...
/**
 * List inbox messages for the current user (for the feed).
 * Parses headers JSON and strips the encrypted body before returning.
 */
export async function getEmailFeed({ limit = 50, status } = {}) {
  const user = await requireAuth();
  try {
    const { listForUser } = await import('../db/email-messages.js');
    const rows = listForUser(user.id, { limit, status });
    return rows.map(m => ({
      ...m,
      headers: typeof m.headers === 'string' ? JSON.parse(m.headers) : m.headers,
      extracted: m.extracted ? JSON.parse(m.extracted) : null,
      bodyEnc: undefined, // never send encrypted body to client
    }));
  } catch (err) {
    console.error('getEmailFeed failed:', err);
    return [];
  }
}

/**
 * Return the decrypted body for a specific message.
 * Verifies the message belongs to the current user before decrypting.
 */
export async function getEmailMessageBody(messageId) {
  const user = await requireAuth();
  try {
    const { getMessageById, getDecryptedBody } = await import('../db/email-messages.js');
    const msg = getMessageById(messageId, user.id);
    if (!msg) return { error: 'Message not found' };
    return { body: getDecryptedBody(messageId) };
  } catch (err) {
    return { error: err.message };
  }
}
...
/**
 * Send a reply to a message via the linked email account.
 * `body` is the plain-text reply the user typed (or the AI-drafted text they approved).
 */
export async function sendEmailReply({ messageId, body }) {
  const user = await requireAuth();
  try {
    const { getMessageById, setStatus, clearMessage } = await import('../db/email-messages.js');
    const { listAccounts } = await import('../db/user-email-accounts.js');
    const { forAccount } = await import('../email/providers/index.js');
    const { logAction } = await import('../db/email-actions.js');
    ...
    if (account.status !== 'active') return { error: `Account is ${account.status} — reconnect it first` };

    const headers = typeof msg.headers === 'string' ? JSON.parse(msg.headers) : msg.headers;
    const mail = forAccount(account);
    const result = await mail.sendMessage({
      threadId: msg.threadId,
      replyToMessageId: msg.providerMessageId,
      to: [headers.from],
    ...
    logAction({
      ...
      payload: { to: headers.from, subject: headers.subject, bodyLength: body.length, ...result },
      mode: 'manual',
      actor: 'user',
    });
    ...
    console.error('sendEmailReply failed:', err);
    return { error: err.message };
  }
}

/**
 * Approve a pending manual/draft action (execute it now) or correct classification.
 */
export async function approveEmailAction(actionId) {
  const user = await requireAuth();
  try {
    const { listForMessage, markAutoExecuted } = await import('../db/email-actions.js');
    const { getMessageById, clearMessage } = await import('../db/email-messages.js');
    const { listAccounts } = await import('../db/user-email-accounts.js');
    const { forAccount } = await import('../email/providers/index.js');

    // Find the action and verify ownership via the message
    const db = (await import('../db/index.js')).getDb();
    const { emailActions } = await import('../db/schema.js');
    const { eq } = await import('drizzle-orm');
    const action = db.select().from(emailActions).where(eq(emailActions.id, actionId)).get();
    if (!action || action.userId !== user.id) return { error: 'Action not found' };

    const msg = getMessageById(action.messageId, user.id);
    if (!msg) return { error: 'Message not found' };
    ...
    switch (action.actionType) {
      case 'archive': await mail.archiveMessage(msg.providerMessageId); break;
      case 'trash':   await mail.trashMessage(msg.providerMessageId); break;
      case 'label':
        await mail.applyLabel(msg.providerMessageId,
          account.provider === 'google' ? 'IMPORTANT' : 'Important'); break;
      default: break;
    }

    markAutoExecuted(actionId);
...
/**
 * Record a user correction on a message classification.
 * Updates the guardrail learning signal and stores the correction for few-shot use.
 */
export async function correctEmailMessage({ messageId, correctedCategory, note }) {
  const user = await requireAuth();
  try {
    const { getMessageById, setClassification } = await import('../db/email-messages.js');
    const { listForMessage, logAction, markCorrected } = await import('../db/email-actions.js');
    const { addCorrection } = await import('../db/email-corrections.js');
    const { recordCorrection, getEffectiveGuardrail } = await import('../db/email-guardrails.js');

    const msg = getMessageById(messageId, user.id);
    if (!msg) return { error: 'Message not found' };

    const headers = typeof msg.headers === 'string' ? JSON.parse(msg.headers) : msg.headers;
    const original = { category: msg.classification, confidence: msg.confidence };
    const corrected = { category: correctedCategory };

    // Find the original classify action to link the correction
    const actions = listForMessage(messageId);
    const classifyAction = actions.find(a => a.actionType === 'classify');

    if (classifyAction) {
      // Log the correction as a new action
      const correctionActionId = logAction({
        messageId, accountId: msg.accountId, userId: user.id,
        actionType: 'classify', payload: { ...corrected, correctedFrom: original },
        mode: 'manual', actor: 'user',
      });
      markCorrected(classifyAction.id, correctionActionId);
    }

    // Store correction for few-shot learning
    addCorrection({
      actionId: classifyAction?.id || messageId,
      userId: user.id,
    ...
    // Update the message classification in DB
    setClassification(messageId, {
      classification: correctedCategory,
      summary: msg.summary,
      confidence: 100, // user-confirmed = certain
      extracted: msg.extracted ? JSON.parse(msg.extracted) : null,
    });

    // Demote guardrail if it auto-classified incorrectly
    const guardrail = getEffectiveGuardrail(user.id, msg.classification, headers.from);
    if (guardrail.guardrailId) recordCorrection(guardrail.guardrailId);
...
/** Return all guardrail rules configured for the current user. */
export async function getEmailGuardrails() {
  const user = await requireAuth();
  try {
    const { listForUser } = await import('../db/email-guardrails.js');
 ...
 * @param {{ scope, scopeValue, mode, confidenceThreshold, undoWindowSec }} opts
 */
export async function upsertEmailGuardrail({ scope, scopeValue = null, mode, confidenceThreshold, undoWindowSec }) {
  const user = await requireAuth();
  try {
    const { upsertGuardrail } = await import('../db/email-guardrails.js');
    const id = upsertGuardrail(user.id, scope, scopeValue, { mode, confidenceThreshold, undoWindowSec });
    return { id };
  } catch (err) {
    return { error: err.message };
  }
...
/** Delete a guardrail rule (reverts that scope to the system default). */
export async function deleteEmailGuardrail(id) {
  const user = await requireAuth();
  try {
    const { getDb } = await import('../db/index.js');
    const { emailGuardrails } = await import('../db/schema.js');
    const { and, eq } = await import('drizzle-orm');
    const db = getDb();
    db.delete(emailGuardrails)
      .where(and(eq(emailGuardrails.id, id), eq(emailGuardrails.userId, user.id)))
      .run();
    return { ok: true };
  } catch (err) {
    return { error: err.message };
