...
import { isSystemEmailConfigured } from '../email/transport.js';
...
export async function requestEmailMfaCode() {
  const session = await auth();
  if (!session?.user?.id) return { error: 'Not signed in' };
  if (!isSystemEmailConfigured()) return { error: 'Email backup is not available' };
  const user = getUserByEmail(session.user.email);
  if (!user) return { error: 'User not found' };
...
export async function isEmailBackupAvailable() {
  return isSystemEmailConfigured();
}
