...
export const middleware = auth((req) => {
  ...
  // Public status page — operational info only (version, uptime, channels
  // configured). No user data, no secrets. Intended for monitoring and
  // future fleet dashboards.
  if (pathname === '/status' || pathname.startsWith('/status/')) return;

  // Password-reset flow — reachable without a session because the whole point
  // is recovering when you can't log in.
  if (pathname === '/forgot-password' || pathname === '/reset-password') return;
  ...
  // MFA gate: an authenticated user whose JWT says mfaRequired but not yet
  // mfaCompleted may ONLY hit the challenge page, the auth endpoints, and
  // sign-out. Everything else redirects to /mfa-challenge until completion.
  if (req.auth?.user?.mfaRequired && !req.auth?.user?.mfaCompleted) {
    if (pathname !== '/mfa-challenge' && pathname !== '/signout') {
