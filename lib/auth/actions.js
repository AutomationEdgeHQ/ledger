...
import { auth, requireAuth, requireAdmin } from './index.js';
import {
  createFirstUser,
  createUser,
  getAllUsers,
...
/**
 * Admin: disable MFA on another user's account. Used when the user has lost
 * access to their authenticator and recovery codes. Logged to console for
 * an audit trail until proper audit logging lands.
 */
export async function adminDisableMfa(id) {
  const admin = await requireAdmin();
  if (id === admin.id) {
    return { error: 'Use your own profile to disable your two-factor setup.' };
  }
  const target = getUserById(id);
  if (!target) return { error: 'User not found.' };
  const ok = disableUserMfa(id);
  if (!ok) return { error: 'Could not disable two-factor.' };
  console.warn(`[audit] admin=${admin.email} disabled MFA on user=${target.email}`);
  return { success: true };
}

/**
 * Update the current user's own profile info (first/last name, nickname).
 * No password verification required — these are low-risk identity fields.
 */
export async function updateProfileInfo({ firstName, lastName, nickname }) {
  const sessionUser = await requireAuth();
  updateUserProfile(sessionUser.id, { firstName, lastName, nickname });
  return { success: true };
}

/**
 * Update the current user's email and/or password.
 * Requires current password for verification (account-takeover sensitive).
 */
