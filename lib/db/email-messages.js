...
/**
 * Get one message by id, scoped to a user (prevents cross-user reads).
 * Returns null if not found or not owned by userId.
 */
export function getMessageById(id, userId) {
  const db = getDb();
  const row = db
    .select()
    .from(emailMessages)
    .where(and(eq(emailMessages.id, id), eq(emailMessages.userId, userId)))
    .get();
  return row ?? null;
}
