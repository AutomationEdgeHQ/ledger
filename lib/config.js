...
const SECRET_KEYS = new Set([
  ...
  'TEAMS_APP_PASSWORD',
  'TEAMS_TENANT_ID',
  // Email agent — per-firm OAuth app client secrets (client IDs / tenant /
  // scopes are non-confidential and live in CONFIG_KEYS below).
...
const CONFIG_KEYS = new Set([
  ...
  'OUTBOUND_SMTP_USER',
  'OUTBOUND_SMTP_FROM',
  'OUTBOUND_SMTP_STREAM',
  'APP_URL',
  // Email agent — per-firm OAuth app config (non-confidential). The firm
  // registers its own Google/Azure app and pastes these in the admin panel.
  // SCOPES are firm-set: the OAuth flow requests exactly what's configured here.
...
export function getConfig(key) {
  ...
  if (value === undefined) {
    const ENV_KEYS = [
      ...
      'APP_URL',
      'APP_HOSTNAME',
      // SMTP — operators set these in .env on the droplet alongside other
      // ops secrets. No admin UI surface for SMTP yet.
      'SYSTEM_SMTP_HOST',
      'SYSTEM_SMTP_PORT',
      'SYSTEM_SMTP_USER',
      'SYSTEM_SMTP_PASS',
